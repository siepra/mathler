...
const Game: React.FC = () => {
  const {puzzle, loading} = useFetchPuzzle();
  ...
  const compareEquasions = useCallback((): boolean => {
    const hasSameCharacters = arraysContainSameElements(
      pendingSolution,
      puzzle.equation,
    );
  ...
  const submitPendingTiles = useCallback(() => {
    ...
    setPendingSolution([]);
  }, [compareEquasions, getTileColor, pendingSolution, board, puzzle]);

  const removeLastPendingCharacter = useCallback(() => {
    setPendingSolution(pendingSolution.slice(0, -1));
  }, [pendingSolution]);

  const addPendingCharacter = useCallback(
    (key: string) => {
      if (pendingSolution.length < 6) {
